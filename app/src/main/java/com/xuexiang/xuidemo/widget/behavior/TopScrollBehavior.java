 ...
 */
public class TopScrollBehavior extends CoordinatorLayout.Behavior<View> {
    ...
    public void onNestedPreScroll(@NonNull CoordinatorLayout parent, @NonNull View child, @NonNull View target, int dx, int dy, @NonNull int[] consumed, int type) {
        if (type == ViewCompat.TYPE_TOUCH) {
            ...
            boolean hideTop = dy > 0 && parent.getScrollY() < child.getHeight();
            //如果子view欲向下滑动，必须要子view不能向下滑动后，才能交给父view滑动
            boolean showTop = dy < 0 && parent.getScrollY() > 0 && !target.canScrollVertically(-1);
            if (hideTop || showTop) {
                int consumedy = dy;
